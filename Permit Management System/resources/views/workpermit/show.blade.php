...
<head>
    ...
    <title>Full Work Permit Application - {{ $application->name }}</title>
    <style>
        .nav-bar {
...
<body>
    ...
    <main class="pagecontent">
        <div class="pageheader">
            <h1>Full Work Permit Application for {{ $application->name }}</h1>
        </div>
        <div class="pagedetails">
            ...
            <p><strong>Employer Name:</strong> {{ $application->employer }}</p>
            <p><strong>Workplace Address:</strong> {{ $application->workplace_address }}</p>
            <p><strong>Employment Duration:</strong> {{ $application->employment_duration }}</p>

            <h2>Documents</h2>
            <p><strong>Application Letter:</strong> <a href="{{ route('image.show', ['filename' => basename($application->app_letter)]) }}" target="_blank">View Document</a></p>
            ...
            <p><strong>Employment Contract:</strong> <a href="{{ route('image.show', ['filename' => basename($application->employment_contract)]) }}" target="_blank">View Document</a></p>
            <p><strong>CV:</strong> <a href="{{ route('image.show', ['filename' => basename($application->cv)]) }}" target="_blank">View Document</a></p>
            <p><strong>Professional Clearance:</strong> <a href="{{ route('image.show', ['filename' => basename($application->professional_clearance)]) }}" target="_blank">View Document</a></p>
            ...
            <div id="rejection-reason-section" style="display: none;" class="rejection-section">
                ...
                <select id="rejection-reason" class="form-control">
                    ...
                    <option value="Invalid Passport">Invalid Passport</option>
                    <option value="Job Position Not Justified">Job Position Not Justified</option>
                    <option value="Missing Employment Contract">Missing Employment Contract</option>
                    <option value="Other">Other</option>
                </select>
                <textarea id="other-reason" placeholder="Enter other reason..." style="display: none;" class="form-control"></textarea>
    ...
    <script>
        ...
        function submitRejection(applicationId) {
            ...
            fetch(`/work-permits/${applicationId}/update-status`, {
                method: 'POST',
                headers: {
        ...
        function updateStatus(status, id) {
            ...
            if (status === 'approved') {
                requestBody.schedule_appointment = true;
            }

            fetch(`/work-permits/${id}/update-status`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
